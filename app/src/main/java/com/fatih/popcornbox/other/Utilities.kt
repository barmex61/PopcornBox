...
import com.fatih.popcornbox.entities.remote.detailresponse.ProductionCompany
import com.fatih.popcornbox.entities.remote.detailresponse.ProductionCountry
...
fun ImageView.setTint(isFavorite:Boolean){
    val color=if (isFavorite) ColorStateList.valueOf(ContextCompat.getColor(this.context,R.color.scarletRed)) else ColorStateList.valueOf(ContextCompat.getColor(this.context,R.color.white))
...
fun <State> MutableList<State>.addFilter(data: State) {
    ...
    if (data == com.fatih.popcornbox.other.State.SEARCH && this.last() == com.fatih.popcornbox.other.State.SEARCH) {
        return
    }
    this.add(data)
